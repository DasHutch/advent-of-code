...
// Goal: Given a string that contains a predicate and value determine if the value is valid
// Assumptions: Each string will have only one separator character
//              Each predicate will be formatted the same way
//              Each value will be lowercase alpha chars only
...
const EXAMPLE_INPUT = [
    ...
    '1-3 b: cdefg', // i
    '2-9 c: ccccccccc', // i
    "5-16 f: ffffffffffffffhf", // i
    "6-8 b: bbbnvbbb", // i
    "6-10 z: zhzzzzfzzzzzzzzzpzz", // i
    "9-13 s: dmssskssqsssssf", // v
    "5-6 v: hvvgvrm", // v
    "1-8 s: sssssssssss", // i
    "1-3 x: kxxx", // v
    "2-5 c: cckgxcnswc", // v
    "10-11 j: rjjpwdzkfjj", // i
    "8-11 r: wkrtlrxcskjglb", // i
    "3-6 l: mqrnhllxx", // v
    "5-6 z: zzzzlg", // i
    "12-13 x: qkrqxtrtsfxpnxg", // i
    "3-7 k: tsbkkkk", // v
    "4-11 j: jjjrjjjjjjjjjjj", // v
    "2-4 c: ccccc", // i
    "2-15 h: hhkhhhhhhhhhhhth", // v
    "7-8 k: hgcjkxhkskk", // v
    "3-10 s: ksxgrkcwfsxrqs", // v
    "10-14 z: hblznxdzxzzzrz", // i
    "5-8 z: mknrjhzzrjzl", // v
    "2-4 v: vqvvjhl", // v
    "3-5 q: fvzqrq", // i
    "9-13 f: flfpkfvffdfcz", // v
    "1-7 b: tbbbbbxbbbbbzbbb", // i
    "6-8 c: ccchvccccccccvzt", // i
    "4-5 w: wwwqww", // v
];
...
// split predicate by ' ' ✅
// - [0] is char count range
// - [1] is char

// filter string chars by char ✅
// compare filtered arr to char count range
const input = [
    "5-16 f: ffffffffffffffhf",
    "6-8 b: bbbnvbbb",
...
const validPasswordCount = input
    .reduce((acc, line) => {
        const [predicate, password] = line.split(':');
        const [characterCountRange, character] = predicate.split(' ');
        const [minCharCount, maxCharCount] = characterCountRange.split('-');
        const passwordChars = password.trim().split('');
        const passwordCharCount = passwordChars.reduce((acc, currentValue) => {  
            return currentValue === character ? acc +=1: acc;
        }, 0);
        const isValid = maxCharCount >= passwordCharCount && minCharCount <= passwordCharCount;
...
const validPasswordCountD2P1 = input
    .reduce((acc, line) => {
        const [predicate, password] = line.split(':');
        const [characterCountRange, character] = predicate.split(' ');
        const [position1, position2] = characterCountRange.split('-');
        const passwordChars = password.trim().split('');
        const charAtPos1 = passwordChars[position1 - 1];
        const charAtPos2 = passwordChars[position2 - 1];
        const chars = [charAtPos1, charAtPos2];
        const requiredCharCount = chars
            .filter((char) => {
                return char === character;
            });
        
        return requiredCharCount.length === 1 ? acc +=1 : acc;
    }, 1);

// day two: part 2
console.log('valid passwords:', validPasswordCountD2P1);
