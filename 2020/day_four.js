...
function validate (passportData) {
    const requiredKeys = ['byr', 'iyr', 'eyr', 'hgt', 'hcl', 'ecl', 'pid'];

    return requiredKeys.every((key) => {
        return passportData.hasOwnProperty(key);
    });
}

const passportData = parse(input);

let validPassportCount = passportData
    .reduce((acc, passportEntry) => {
        return validate(passportEntry) ? acc += 1 : acc;
    }, 0);
...
function validateBirthYear (data) {
    const birthYearKey = 'byr';
    const minBirthYear = 1920;
    const maxBirthYear = 2002;
    const birthYearRequiredDigits = 4;
    const birthYear = data[birthYearKey];

    return _validateNumberLength(birthYear, birthYearRequiredDigits) &&
    _validateNumberRange(birthYear, minBirthYear, maxBirthYear);
}

function validateIssueYear (data) {
    const issueYearKey = 'iyr';
    const issueYearRequiredDigits = 4;
    const minIssueYear = 2010;
    const maxIssueYear = 2020;
    const issueYear = data[issueYearKey];

    return _validateNumberLength(issueYear, issueYearRequiredDigits) &&
    _validateNumberRange(issueYear, minIssueYear, maxIssueYear);
}

function validateExpirationYear (data) {
    const expirationYearKey = 'eyr';
    const expirationYearRequiredDigits = 4;
    const minExpirationYear = 2020;
    const maxExpirationYear = 2031;
    const expirationYear = data[expirationYearKey];

    return _validateNumberLength(expirationYear, expirationYearRequiredDigits) &&
    _validateNumberRange(expirationYear, minExpirationYear, maxExpirationYear);
}
...
function validateHeight (data) {
    ...
    const height = heightData.substr(0, heightData.length - 2);
    const measurement = heightData.substr(heightData.length - 2, 2);

    if (isNaN(parseInt(height, 10))) {
        return false;
    }

    if (measurement == MEASUREMENT_TYPE.CM) {
        return _validateNumberRange(height, MEASUREMENT_RANGES.CM.MIN, MEASUREMENT_RANGES.CM.MAX);
    } else if (measurement == MEASUREMENT_TYPE.IN) {
        return _validateNumberRange(height, MEASUREMENT_RANGES.IN.MIN, MEASUREMENT_RANGES.IN.MAX);
    } else {
...
function validateHairColor (data) {
    const hairColorKey = 'hcl';
    const hairColor = data[hairColorKey];
    const regex = /^#[0-9a-fA-F]{6}$/;

    return regex.test(hairColor);
}

function validateEyeColor (data) {
    const allowed = ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'];
    const eyeColorKey = 'ecl';
    const eyeColor = data[eyeColorKey];
    return allowed.includes(eyeColor);
}

function validatePassportId (data) {
    const passportIdKey = 'pid';
    const passportId = data[passportIdKey];
    const passportIdRequiredDigits = 9;
    const regex = /^[0-9]{9}$/i;

    return _validateNumberLength(passportId, passportIdRequiredDigits) && 
    regex.test(passportId);
}

function validateCountryId (data) {
    return true;
}

function _validateNumberRange (number, min, max) {
    return number >= min && number <= max;
}

function _validateNumberLength (number, length) {
    return number.length === length;
}

validPassportCount = passportData
    .filter(validate)
    .filter(validateBirthYear)
    .filter(validateIssueYear)
    .filter(validateExpirationYear)
    .filter(validateHeight)
    .filter(validateHairColor)
    .filter(validateEyeColor)
    .filter(validatePassportId)
    .filter(validateCountryId)
    .reduce((acc, passportEntry) => {
        return validate(passportEntry) ? acc += 1 : acc;
    }, 0);
